$(document).ready(function () {
    ...
    connection.start().then(function () {
        ...
        connection.invoke('AddIdentityName', userName).then(() => {
        });
    ...
    function AppViewModel() {
        ...
        self.sendToRoom = function (roomName, message) {
            if (roomName.length > 0 && message.length > 0) {
                fetch('https://localhost:5001/api/Messages', {
                    ...
                    body: JSON.stringify({ room: roomName, content: message, userName: userName })
                });
            }
        }
        ...
        self.editRoom = function () {
            ...
            fetch('https://localhost:5001/api/Rooms/' + roomId, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id: roomId, name: roomName, userName: userName })
            });
        }

        self.deleteRoom = function () {
            fetch('https://localhost:5001/api/Rooms/' + self.joinedRoomId()+"/" + userName, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id: self.joinedRoomId(), userName: userName})
            });
        }
        ...
        self.deleteMessage = function () {
            ...
            fetch('https://localhost:5001/api/Messages/' + messageId +"/" + userName, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id: messageId, userName: userName })
            });
        }

        self.messageHistory = function () {
            fetch('https://localhost:5001/api/Messages/Room/' + viewModel.joinedRoom())
                .then(response => response.json())
                .then(data => {
        ...
        self.uploadFiles = function () {
            ...
            $.ajax({
                type: "POST",
                url: 'https://localhost:5001/api/Uploads',
